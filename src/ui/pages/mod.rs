...
impl Page for HomePage {
    fn draw_page(&self) -> Result<()> {
        ...
        let epics: Vec<_> = Itertools::sorted(db_state.epics.iter()).collect();
        for (id, epic) in epics {
            let id_str = id.to_string();
            let name_str = &epic.name;
            let status_str = format!("{}", epic.status);

            println!(
                "{} | {} | {}",
                get_column_string(&id_str, 10),
                get_column_string(name_str, 30),
                get_column_string(&status_str, 15)
            );
        }
...
impl Page for EpicDetail {
    fn draw_page(&self) -> Result<()> {
        ...
        let id_str = self.epic_id.to_string();
        let name_str = &epic.name;
        let description_str = &epic.description;
        let status_str = format!("{}", epic.status);

        println!(
            "{} | {} | {} | {}",
            get_column_string(&id_str, 6),
            get_column_string(name_str, 20),
            get_column_string(description_str, 30),
            get_column_string(&status_str, 15)
        );
        ...
        // Ordena las historias por ID utilizando itertools::sorted
        let sorted_stories: Vec<_> = itertools::sorted(stories.iter(), |story| {
            db_state.stories
                .iter()
                .find_map(|(id, s)| if s == *story { Some(*id) } else { None })
                .unwrap()
        }).collect();
        ...
        for story in sorted_stories {
            let id_str = db_state
                ...
                .unwrap()
                .to_string();
            let name_str = &story.name;
            let status_str = format!("{}", story.status);
...
impl Page for StoryDetail {
    fn draw_page(&self) -> Result<()> {
        ...
        let id_str = self.story_id.to_string();
        let name_str = &story.name;
        let description_str = &story.description;
        let status_str = format!("{}", story.status);
