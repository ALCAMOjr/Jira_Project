...
impl JiraDatabase {
    ...
    pub fn create_story(&self, story: Story, epic_id: u32) -> Result<u32> {
        let mut db_state = self.read_db()?;
        let epic = db_state.epics.get_mut(&epic_id).ok_or_else(|| anyhow!("Epic not found"))?;
        let new_id = db_state.last_item_id + 1;
        db_state.last_item_id = new_id;
    ...
    pub fn delete_epic(&self, epic_id: u32) -> Result<()> {
        let mut db_state = self.read_db()?;
        let epic = db_state.epics.remove(&epic_id).ok_or_else(|| anyhow!("Epic not found"))?;
    ...
    pub fn delete_story(&self, epic_id: u32, story_id: u32) -> Result<()> {
        let mut db_state = self.read_db()?;
        let epic = db_state.epics.get_mut(&epic_id).ok_or_else(|| anyhow!("Epic not found"))?;
        if epic.stories.iter().position(|&id| id == story_id).is_none() {
            return Err(anyhow!("Story not found in the specified Epic"));
        }
    ...
    pub fn update_story_status(&self, story_id: u32, status: Status) -> Result<()> {
        let mut db_state = self.read_db()?;
        let story = db_state.stories.get_mut(&story_id).ok_or_else(|| anyhow!("Story not found"))?;
        story.status = status;
        self.write_db(&db_state)?;
        Ok(())
    }
...
impl Database for JSONFileDatabase {
    ...
    fn write_db(&self, db_state: &DBState) -> Result<()> {
        let data = serde_json::to_vec(db_state)
            .context("Failed to serialize DBState to JSON")?;
        fs::write(&self.file_path, &data)
            .with_context(|| format!("Failed to write JSON to file: {}", &self.file_path))?;
        Ok(())
    }
